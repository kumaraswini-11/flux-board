...
// NOTE: Next.js doesn’t provide access to the user's IP address natively, so Arcjet created the Arcjet IP Detection library, which provides a utility to find the public IP of a request.
...
const emailOptions = {
  ...
  // Block emails that are disposable, invalid, or have no MX records (types of emails i want to block)
...
const botOptions = {
  ...
  allow: [], // prevents bots from submitting the form, if you want to allow some bots, you can add them here
} satisfies BotOptions;

// Sliding window algorithm tracks the number of requests made by a client over a sliding window so that the window moves with time.
